...
export type AsCamelCase<R extends Record<string, any> | Readonly<Record<string, any>>> = {
    [K in keyof R as SnakeToCamelCase<K & string>]: R[K];
};

/** Converts an object type or interface to the same type with snake case keys */
export type AsSnakeCase<R extends Record<string, any> | Readonly<Record<string, any>>> = {
    [K in keyof R as CamelToSnakeCase<K & string>]: R[K];
};
